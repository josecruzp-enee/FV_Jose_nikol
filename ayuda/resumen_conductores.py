...
def _is_internal_import(mod: str) -> bool:
    ...
    return mod.startswith(".") or mod.startswith("electrical")
...
def main() -> None:
    repo_root = os.getcwd()
    conductores_dir = os.path.join(repo_root, "electrical", "conductores")
    if not os.path.isdir(conductores_dir):
        raise SystemExit(f"No existe carpeta: {conductores_dir}")

    files = _iter_py_files(conductores_dir)
    infos = [_scan_file(repo_root, p) for p in files]

    # Matriz simple de dependencias por imports
    ...
    # Reporte
    lines: List[str] = []
    lines.append("# Resumen automático — electrical/conductores\n\n")
    ...
    for fi in infos:
        ...
        if fi.calls:
            calls = sorted(list(fi.calls))[:50]
            lines.append(f"- Llamadas detectadas (muestra ≤50): {', '.join(f'`{c}`' for c in calls)}\n")
        lines.append("\n")
    ...
    out_path = os.path.join(repo_root, "resumen_conductores.md")
