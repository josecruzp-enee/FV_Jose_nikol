...
def generar_layout_paneles(
    ...
    *,
    ...
    if dos_aguas and n >= 6:
        ...
        W_der, H_der = 0.0, 0.0
        W, H = W_izq, H_izq
        rows_obj = rows_izq  # para consistencia
    ...
    ax.add_patch(Rectangle((0, 0), roof_W, roof_H, linewidth=1.2, edgecolor="#98A2B3", facecolor="#F2F4F7"))

    off_x = margen_techo_m
    off_y = margen_techo_m

    ax.add_patch(Rectangle((off_x, off_y), W, H, linewidth=1.1, edgecolor="#667085", facecolor="none"))

    if dos_aguas and n_der > 0:
        x_c = off_x + W_izq
        ax.add_patch(Rectangle((x_c, off_y), gap_cumbrera_m, H, linewidth=0.0, edgecolor="none", facecolor="#FFFFFF"))
    ...
    def _dibujar_grid(n_local, cols_local, rows_local, x0, y0, start_num):
        ...
        for r in range(rows_local):
            for c in range(cols_local):
                ...
                ax.add_patch(Rectangle((x, y), panel_w_m, panel_h_m, linewidth=0.8, edgecolor="#0B2E4A", facecolor="#1F2A37"))
                ax.text(x + panel_w_m / 2, y + panel_h_m / 2, str(start_num + k), ha="center", va="center", fontsize=7, color="white")
                k += 1
        return start_num + k

    next_num = _dibujar_grid(n_izq, cols_izq, rows_izq, off_x, off_y, start_num=1)

    if dos_aguas and n_der > 0:
        x0_der = off_x + W_izq + gap_cumbrera_m
        _dibujar_grid(n_der, cols_der, rows_obj, x0_der, off_y, start_num=next_num)
