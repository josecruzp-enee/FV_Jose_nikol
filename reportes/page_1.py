...
def p1_tabla_solucion_unica(datos, kwp, capex, ds, estado, pal, content_w):
    ...
    prod_anual = float(kwp) * float(datos.prod_base_kwh_kwp_mes) * 12.0
    n_paneles = int(math.ceil(float(kwp) * 1000.0 / 550.0))
    panel_wp = 550
    ...
    data = [
        ...
        ["Cobertura objetivo", f"{datos.cobertura_objetivo*100:.0f}%",
         "Financiamiento", f"{datos.tasa_anual*100:.2f}% | {int(datos.plazo_anios)} años | {datos.porcentaje_financiado*100:.0f}%"],
        ["Sistema", f"{num(kwp,2)} kWp",
         "CAPEX", money_L(capex)],
        ["Producción anual est.", f"{prod_anual:,.0f} kWh/año",
         "DSCR", num(ds,2)],
        ["Módulos FV", f"{n_paneles} × {int(panel_wp)} Wp",
         "Estado", estado_txt],
    ]
...
def p1_tabla_decision(decision, cuota_mensual, plazo_anios, pal, content_w):
    ...
    pago_actual = float(decision["pago_actual"])
    pago_residual = float(decision.get("pago_residual", 0.0))
    cuota = float(decision.get("cuota_fv", cuota_mensual))
    pago_total = float(decision.get("pago_total_fv", pago_residual + cuota))
    ahorro = float(decision.get("ahorro_mensual", pago_actual - pago_total))
...
def build_page_1(resultado: Dict[str, Any], datos, pal, styles):
    """
    ...
    kwp = float(sizing["kwp_recomendado"])
    capex = float(sizing["capex_L"])
    ds = float(eval_.get("dscr", 0.0))
    impacto = float(decision.get("ahorro_mensual", 0.0))
    peor = float(eval_.get("peor_mes", 0.0))
    estado = str(eval_.get("estado", ""))
    ...
    story += p1_tabla_decision(decision, resultado["cuota_mensual"], datos.plazo_anios, pal, content_w)
    story += p1_conclusion(impacto, ds, peor, kwp, datos.cobertura_objetivo, estado, pal, content_w)
