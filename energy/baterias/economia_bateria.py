...
def _estado_escenario(
    *,
    ...
    if payback_bateria is None or payback_bateria > vida_util:
        return "NO RENTABLE"
    ...
    *,
    ...
    candidatos = [
        ...
        if (
            ...
            and escenario.payback_bateria_anios <= vida_util_bateria_anios
        )
    ]

    if not candidatos:
        escenario_base.criterio_seleccion = (
            "Ninguna batería recupera su inversión dentro de su vida útil."
        )
    ...
    mejor = max(
        ...
        key=lambda escenario: (
            ...
            - escenario.capex_bateria_l / max(vida_util_bateria_anios, 1),
            -escenario.capex_bateria_l,
        ),
    )
    mejor.criterio_seleccion = (
        "Escenario seleccionado por beneficio económico incremental neto, "
        "considerando el valor de la energía inyectada y la recuperación "
