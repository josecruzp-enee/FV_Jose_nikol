...
from energy.baterias.entrada_bateria import EntradaBateriafrom energy.baterias.resultado_bateria import (EscenarioBateria,ResultadoBateria,)

def _normalizar_12m(valores, nombre: str) -> List[float]:if not valores or len(valores) != 12:raise ValueError(f"{nombre} debe contener 12 valores.")

return [max(0.0, float(valor or 0.0)) for valor in valores]

def energia_descargada_diaria(resultado: ResultadoBateria | None,) -> float:if resultado is None:return 0.0
...
def construir_energia_util_12m(*,entrada: EntradaBateria,resultado: ResultadoBateria | None,) -> List[float]:if resultado is None:raise ValueError("El escenario económico requiere un resultado técnico.")
...
def calcular_capex_bateria_l(*,capacidad_kwh: float,entrada: EntradaBateria,) -> float:return (max(0.0, float(capacidad_kwh))* entrada.costo_bateria_l_kwh)

def calcular_cuota_mensual(*,capex_total_l: float,entrada: EntradaBateria,) -> float:if entrada.modo_financiamiento == "contado":return 0.0
...
def calcular_om_mensual(*,capex_total_l: float,entrada: EntradaBateria,) -> float:return (max(0.0, float(capex_total_l))* max(0.0, float(entrada.om_anual_pct))/ 12.0)

def simular_finanzas_12m(*,entrada: EntradaBateria,resultado: ResultadoBateria,cuota_mensual_l: float,om_mensual_l: float,) -> List[Dict[str, float]]:demanda = _normalizar_12m(resultado.demanda_12m_kwh,"demanda_12m_kwh",)compra = _normalizar_12m(resultado.compra_red_con_bateria_12m_kwh,"compra_red_con_bateria_12m_kwh",)inyeccion = _normalizar_12m(resultado.excedente_con_bateria_12m_kwh,"excedente_con_bateria_12m_kwh",)
...
def _calcular_dscr(*,tabla_12m: List[Dict[str, float]],cuota_mensual_l: float,) -> Optional[float]:if cuota_mensual_l <= 0:return None
...
def _totales_red(tabla_12m):costo_compra = sum(fila["costo_compra_red_l"]for fila in tabla_12m)ingreso_inyeccion = sum(fila["ingreso_inyeccion_l"]for fila in tabla_12m)
...
def evaluar_escenario_bateria(*,entrada: EntradaBateria,nombre: str,capacidad_kwh: float,potencia_kw: float,resultado_tecnico: ResultadoBateria | None,energia_objetivo_kwh: float = 0.0,escenario_base: EscenarioBateria | None = None,) -> EscenarioBateria:if resultado_tecnico is None:raise ValueError("El escenario requiere una simulación técnica 8760.")
