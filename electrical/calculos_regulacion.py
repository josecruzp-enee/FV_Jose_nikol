...
from electrical.cables_conductores import (
    calibres, ampacidad, resistencia_cu_ohm_km, tabla_base, idx_calibre
)
...
def _max_calibre(c1: str, c2: str, *, tipo: str) -> str:
...
def elegir_calibre(*, v: float, i: float, i_diseno_a: float, l_m: float, vd_obj_pct: float, tipo: str, n_hilos: int) -> str:
    ...
    return _max_calibre(g_amp, g_vd, tipo=tipo)

# ==========================================================
# Tramos listos para usar (DC/AC)
# ==========================================================
...
# ==========================================================
# Versión “base NEC” (sin derating aquí)
# (Si luego quieres NEC real: aquí solo usamos tabla_base(material))
# ==========================================================

def tramo_tabla_base(
    *, nombre: str, material: str, i_a: float, v_v: float, l_m: float, vd_obj_pct: float, n_hilos: int
) -> Dict[str, float | str | bool]:
    tab = tabla_base(material)
    if i_a <= 0 or v_v <= 0 or l_m <= 0:
        return {"nombre": nombre, "ok": False, "nota": "Datos insuficientes."}
    awg = _elegir_por_tabla(tab, i_a=i_a, v_v=v_v, l_m=l_m, vd_obj_pct=vd_obj_pct, n_hilos=n_hilos)
    r = _r_de_tabla(tab, awg)
...
def _r_de_tabla(tab: List[Dict[str, float]], awg: str) -> float:
    for t in tab:
        if str(t["awg"]) == str(awg):
            return float(t["r_ohm_km"])
    return float(tab[-1]["r_ohm_km"])

def _elegir_por_tabla(tab: List[Dict[str, float]], *, i_a: float, v_v: float, l_m: float, vd_obj_pct: float, n_hilos: int) -> str:
    cand = _primero_por_amp(tab, i_a=i_a)
    return _mejorar_por_vd(tab, awg=cand, i_a=i_a, v_v=v_v, l_m=l_m, vd_obj_pct=vd_obj_pct, n_hilos=n_hilos)

def _primero_por_amp(tab: List[Dict[str, float]], *, i_a: float) -> str:
    for t in tab:
        if float(i_a) <= float(t["amp_a"]):
            return str(t["awg"])
    return str(tab[-1]["awg"])

def _mejorar_por_vd(tab: List[Dict[str, float]], *, awg: str, i_a: float, v_v: float, l_m: float, vd_obj_pct: float, n_hilos: int) -> str:
